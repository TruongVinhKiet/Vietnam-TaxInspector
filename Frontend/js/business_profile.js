/**
 * business_profile.js  –  Trang Tài khoản Người nộp thuế (Security Hardened + Signature 2FA)
 * =========================================================================================
 * Features:
 *   1. Cookie-based auth via secureFetch()
 *   2. Face / CCCD / Signature setup & reset for taxpayers
 *   3. Phone number inline edit
 *   4. Canvas-based signature drawing
 */

const ROLE_MAP = {
    taxpayer:   { label: 'Người nộp thuế', color: 'bg-emerald-100 text-emerald-700', dot: 'bg-emerald-500' },
    enterprise: { label: 'Doanh nghiệp', color: 'bg-teal-100 text-teal-700', dot: 'bg-teal-500' },
};
...
function renderProfile(user) {
    const role = ROLE_MAP[user.role] || { label: 'Người nộp thuế', color: 'bg-emerald-100 text-emerald-700', dot: 'bg-emerald-500' };
    ...
    document.getElementById('info-department').textContent = user.department || 'Chưa thiết lập';
...
function setProfileAvatar(avatarData, initials) {
    ...
    avatarText.textContent = initials || 'NNT';
}
